...
from erpnext.setup.doctype.employee.employee import create_user as create_employee_user
...
class JobOffer(Document):
	...
	def on_submit(self):
		self.create_employee_and_user_if_accepted()
	...
	def create_employee_and_user_if_accepted(self) -> None:
		if self.status != "Accepted" or self.docstatus != 1:
			return

		employee = frappe.db.get_value("Employee", {"job_applicant": self.job_applicant}, "name")
		created_employee = False

		if not employee:
			employee_doc = make_employee(self.name)
			employee_doc.job_applicant = self.job_applicant
			employee_doc.company = self.company
			employee_doc.designation = self.designation

			if not employee_doc.employee_name and self.applicant_name:
				employee_doc.employee_name = self.applicant_name
			if not employee_doc.first_name and self.applicant_name:
				employee_doc.first_name = self.applicant_name.split(" ")[0] or self.applicant_name
			if self.offer_date and not employee_doc.date_of_joining:
				employee_doc.date_of_joining = self.offer_date
		...
		if employee:
			...
			if not user:
				email = self.applicant_email or frappe.db.get_value(
				...
				if email:
					if frappe.db.exists("User", email):
						...
						user = create_employee_user(employee, email=email)
						created_user = True

		if created_employee or created_user:
			links = []
			if employee:
				links.append(get_link_to_form("Employee", employee))
			if user:
				links.append(get_link_to_form("User", user))
			message = _("Job offer accepted. Please complete details for {0}.").format(" / ".join(links))
