...
// Auto-refresh list view after saves across the app.
(() => {
    if (frappe.__auto_list_refresh_global_setup) return;

    const setup = () => {
        if (frappe.__auto_list_refresh_global_setup) return;
        if (!frappe.ui || !frappe.ui.form || !frappe.ui.form.ScriptManager) {
            setTimeout(setup, 500);
            return;
        }

        frappe.__auto_list_refresh_global_setup = true;

        const originalTrigger = frappe.ui.form.ScriptManager.prototype.trigger;
        if (!originalTrigger) return;

        frappe.ui.form.ScriptManager.prototype.trigger = function (event_name, doctype, name) {
            const result = originalTrigger.apply(this, arguments);

            if (event_name === "after_save") {
                const dt = doctype || (this.frm && this.frm.doctype);
                if (dt) {
                    frappe.__pending_list_refresh_doctype = dt;
                    if (window.cur_list && cur_list.doctype === dt && frappe.get_route()[0] === "List") {
                        cur_list.last_args = null;
                        cur_list.refresh();
                        frappe.__pending_list_refresh_doctype = null;
                    }
                }
            }
        ...
        frappe.router.on("change", () => {
            const dt = frappe.__pending_list_refresh_doctype;
            if (!dt) return;
            if (!window.cur_list) return;
            if (frappe.get_route()[0] !== "List") return;
            if (cur_list.doctype !== dt) return;
